...
- `handoff` 追記で `handoff.md` は必ず変更されるため、「repo clean」と書く場合は **「handoff.md 以外 clean」** のように補足する
- `Now` の状態宣言は **Git スナップショットと一致**させる
- CLI引数はシェル展開されるため、`--next` にバッククォートを使わない（必要なら単一引用符か対話入力）
...
- **push 状態の誤記**: `push済み` と書いたが `git push` は未実行
- **データ混入**: `*.csv` / `*.xlsx` の追跡
