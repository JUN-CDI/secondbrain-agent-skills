...
| Handoff（ログ） | [[System/Documentation/引き継ぎ]] | 前回までの状態を復元したい |
...
- Agent Skills（Codex/Cursor）は `System/Skills/`（別Git repo / このVault repoでは追跡しない）を原本にして運用（Codexは `sync-codex-skills.sh` で `~/.codex/skills` へ同期、未取得なら `--clone` / `--from-codex`）
