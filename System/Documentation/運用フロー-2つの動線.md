...
flowchart TB
    ...
    subgraph DevFlow["開発フロー"]
        ...
        D3 --> D4["CLAUDE.md/AGENTS.md/handoff.md/docs/"]
        D4 --> D5[開発作業]
        D5 --> D6[Git commit]
        D6 --> D7{完了?}
        D7 -->|NO| D5
...
├── AGENTS.md      # Codex用ルール
├── handoff.md     # 再開SSOT（handoff / endwork）
├── README.md      # プロジェクト概要
├── .gitignore     # Git除外設定
├── .vscode/        # エディタ用（ローカル）
...
#### Step 5.5: 引き継ぎ（必要時）
- 再開SSOT: repo直下の `handoff.md`
  - 切替直前/不安時: `handoff quick`（Nextだけ入力）
  - その日の終わり: `endwork`（`handoff full` + `handoff.md` のみcommit）
- 補助（任意）: [[引き継ぎ-開発作業]] / Claude Code Skill: `dev-handoff`
...
| 引き継ぎ（切替用） | `handoff` / `handoff quick` | repo直下 `handoff.md` を更新（Nextだけ） |
| 引き継ぎ（終了用） | `endwork` | `handoff full` + `handoff.md` のみcommit |
...
# handoff / endwork インストール（初回）
./System/Scripts/install-handoff.sh
